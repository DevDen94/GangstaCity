...
namespace DialogueQuests
{
    ...
    public class NarrativeControls : MonoBehaviour
    {
        ...
        {
            //Key controls
            if (keyboard_controls && talk_key != KeyCode.None && ControlFreak2.CF2Input.GetKeyDown(talk_key))
                OnPressTalk(false);
            if (keyboard_controls && journal_key != KeyCode.None && ControlFreak2.CF2Input.GetKeyDown(journal_key))
                OnPressJournal();
            if (keyboard_controls && cancel_key != KeyCode.None && ControlFreak2.CF2Input.GetKeyDown(cancel_key))
                OnPressCancel(false);
            if (mouse_controls && ControlFreak2.CF2Input.GetMouseButtonDown(0))
                OnPressTalk(true);
            if (mouse_controls && ControlFreak2.CF2Input.GetMouseButtonDown(1))
                OnPressCancel(true);
