    ...
    public class SaveSystemTestMenu : MonoBehaviour
    {
        ...
        void HandleCursor(bool open)
        {
            if (allowCursorWhileOpen && InputDeviceManager.deviceUsesCursor)
            {
                if (open)
                {
                    m_prevCursorState = ControlFreak2.CFCursor.visible;
                    ControlFreak2.CFCursor.visible = true;
                    ControlFreak2.CFCursor.lockState = CursorLockMode.None;
                }
                else
                {
                    ControlFreak2.CFCursor.visible = m_prevCursorState;
                    ControlFreak2.CFCursor.lockState = m_prevCursorState ? CursorLockMode.None : CursorLockMode.Locked;
                }
            }
        }
