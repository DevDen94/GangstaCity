...
namespace PixelCrushers.DialogueSystem.Demo
{
    ...
    public class NavigateOnMouseClick : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (ControlFreak2.CF2Input.GetMouseButtonDown((int)mouseButton) && GUIUtility.hotControl == 0)
            {
                Plane playerPlane = new Plane(Vector3.up, m_myTransform.position);
                Ray ray = UnityEngine.Camera.main.ScreenPointToRay(ControlFreak2.CF2Input.mousePosition);
                float hitdist = 0.0f;
            ...
            else if (ControlFreak2.CF2Input.GetMouseButton((int)mouseButton) && GUIUtility.hotControl == 0)
            {
