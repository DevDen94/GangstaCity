...
public class RTC_MainCamera : MonoBehaviour{
	...
	void Inputs(){
		...
		case RTC_Settings.ControllerType.Keyboard:
			...
			orbitX += ControlFreak2.CF2Input.GetAxis (RTCSettings.mainGunXInput) * (orbitXSpeed * 10f) * Time.deltaTime;
			orbitY -= ControlFreak2.CF2Input.GetAxis (RTCSettings.mainGunYInput) * (orbitYSpeed * 10f) * Time.deltaTime;

			if(ControlFreak2.CF2Input.GetKeyDown(RTCSettings.changeCameraKB))
				ChangeCamera();
