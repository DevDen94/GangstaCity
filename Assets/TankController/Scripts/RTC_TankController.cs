...
public class RTC_TankController : MonoBehaviour {
	...
	void Inputs(){
		...
		case RTC_Settings.ControllerType.Keyboard:
			...
			// Motor input.
			gasInput = ControlFreak2.CF2Input.GetAxis (RTCSettings.gasInput);
			// Steering input.
			steerInput = ControlFreak2.CF2Input.GetAxis (RTCSettings.steerInput);
			// Brake input
			brakeInput = -Mathf.Clamp (ControlFreak2.CF2Input.GetAxis (RTCSettings.gasInput), -1f, 0f);
			// Headlights.

			if (ControlFreak2.CF2Input.GetKeyDown (RTCSettings.headlightsKB))
				headLightsOn = !headLightsOn;

			if (ControlFreak2.CF2Input.GetKeyDown (RTCSettings.startEngineKB)) {
				if (!engineRunning)
					StartEngine ();
				else
					KillEngine ();
