...
public class RTC_TankGunController : MonoBehaviour {
	...
	void Inputs(){
		...
		case RTC_Settings.ControllerType.Keyboard:
			...
			case AimType.Direct:
				steerInput = Mathf.Lerp(steerInput, ControlFreak2.CF2Input.GetAxis (RTCSettings.mainGunXInput) * horizontalSensitivity, Time.deltaTime * 10f);
				elevatorInput = Mathf.Lerp(elevatorInput, -ControlFreak2.CF2Input.GetAxis (RTCSettings.mainGunYInput) * verticalSensitivity, Time.deltaTime * 10f);
				break;

			}

			if (ControlFreak2.CF2Input.GetKeyDown (RTCSettings.fireKB)) {
				Fire ();
			}

			if(ControlFreak2.CF2Input.GetKeyDown(RTCSettings.changeAmmunation)){
